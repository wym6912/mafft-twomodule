...
void readData_pointer2( FILE *fp, int nseq, char **name, int *nlen, char **seq )
{
	...
	if( outnumber )
	{
		char *namebuf;
		char *cptr;
		namebuf = calloc( BLEN+100, sizeof( char ) );
		for( i=0; i<nseq; i++ )
		{
			namebuf[0] = '=';
...
void WriteGapFill( FILE *fp, int locnjob, char name[][BLEN], int nlen[M], char **aseq )
{
	static char b[NLEN];
	int i, j;
	int nalen[M];
...
void writeData( FILE *fp, int locnjob, char name[][BLEN], int nlen[], char **aseq )
{
	int i, j;
	int nalen;
...
void writeData_reorder( FILE *fp, int locnjob, char name[][BLEN], int nlen[], char **aseq, int *order )
{
	int i, j, k;
	int nalen;
