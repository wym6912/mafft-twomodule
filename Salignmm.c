...
void init__protein_score(double **cpmx1, double **cpmx2, int len1, int len2, int band, double **amino_dis, int *query_place, double *ans)
{
	int i, j, k, l, h;
	...
	for(i = 0; i <= len1; ++ i)
		for(j = 0; j <= len2; ++ j)
		{
			...
			{
				reporterr("(%d %d): %f\n", i, j, matrix_query(i, j, band, ans, query_place));
					...
					if(fabs(cpmx1[k][i]) > eps) 
					{
						...
						for(l = 0; l < nalphabets; ++ l) 
							if(fabs(cpmx2[l][j]) > eps) 
								reporterr("cpmx2[%d][%d] = %f, number = %f\n", k, j, cpmx2[l][j], amino_dis[(unsigned char)amino[k]][(unsigned char)amino[l]]);
					}
				}
				  ...
				  char *r1, char *r2, double **ad, double hgp1, double hgp2, int headgp, int tailgp)
{
	...
	*/
	...
	init__protein_score(cpmx1, cpmx2, len1, len2, band, ad, query_vec, homo_matrix);
	...
	if(headgp == 1)
	{
		for(i = 1; i <= len1; ++ i) matrix_update(i, 0, band, *og1 * hgp2 + *(fg1 + i - 1) * *gf2, val_vec, query_vec);
		for(i = 1; i <= len2; ++ i) matrix_update(0, i, band, *og2 * hgp1 + *(fg2 + i - 1) * *gf1, val_vec, query_vec);
	}
	...
	{
		*mdjp = matrix_query(0, i, band, val_vec, query_vec); // + *(og1 + 1) * *(gf2 + i - 1);
		*mpjp = 0;
	}
	for(i = 1; i <= len1; ++ i)
	{
		max_data_i = matrix_query(i - 1, MAX(i - band, 0), band, val_vec, query_vec) + *(og2 + MAX(i - band, 0) + 1) * *(gf1 + i - 1);
	...
	for(i = 0; i < len1 + 1; ++ i, reporterr("\n%-6d", i))
		for(j = 0; j < len2 + 1; ++ j, reporterr(" "))
		{
			if(insideband(i, j, band))
			{
				reporterr("%7.1f", matrix_query(i, j, band, val_vec, query_vec));
			}
			else
			{
				reporterr("???????");
...
# endif
    ...
    static TLS int length1, length2, mxlength, tmplen, band, diff;
...
#if 1
	val = Kband__MSA(icyc, jcyc, length1, length2, band + diff, opgap1f, fgap1f, opgap2f, fgap2f, gapf1qp, gapf2qp, 
	...
	while(band <= mxlength)
	{
		...
		val = Kband__MSA(icyc, jcyc, length1, length2, band + diff, opgap1f, fgap1f, opgap2f, fgap2f, gapf1qp, gapf2qp, 
		                 cpmx1, cpmx2, gap1, gap2, amino_dynamicmtx, headgapfreq1, headgapfreq2, headgp, tailgp);
#if DEBUG
		printf("Val = %f, band = %d, length1 = %d, length2 = %d\n", val, band, length1, length2);
#endif
		if(val == -inf)
		{
			band <<= 1;
		}
		else if(val == oldval) break;
		else
		{
			oldval = val;
			band <<= 1;
